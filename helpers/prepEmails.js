module.exports = {
  ...
  prepHyperEmail: function(sensor) {
    const sizeUnit = sensor.HyperSizeUnit === 1 ? 'in' : 'mm';
    const weightUnit = sensor.HyperWeightUnit === 1 ? 'oz' : 'g';
    const weightMinusLensUnit = sensor.HyperWeightMinusLensUnit === 1 ? 'oz' : 'g';
    let sensorHTML = '';
    ...
    sensorHTML += `<tr><td>Weight (minus lens)</td><td>${sensor.HyperWeightMinusLens} ${weightMinusLensUnit}</td></tr>`;
    sensorHTML += '</table>';
    return sensorHTML;
  },
  prepLidarEmail: function(sensor) {
  ...
  prepThermalEmail: function(sensor) {
